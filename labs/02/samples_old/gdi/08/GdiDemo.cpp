...
const TCHAR g_title[] = "Gdi demo";					// Текст в заголовке окна
const TCHAR g_windowClass[] = "GdiDemoClass";		// Имя класса окна
const TCHAR g_helloWorld[] = "Работа со шрифтами и их свойствами!";		// строка, для вывода на поверхность окна
const UINT DRAW_TIMER_ID = 1;
UINT		g_drawTimerId = 0;		// идентификатор таймера, который будет вызывать перерисовку нашего окна

const float CONST_PI = 3.1415927f;
...
int g_timoshenkoFlyPhaseX = 0;
int g_timoshenkoFlyPhaseY = 0;
...
void InitGraphicResources(HWND hWnd)
{
	...
	g_timoshenkoBitmap = LoadBitmap(g_hInstance, MAKEINTRESOURCE(IDB_TIMOSHENKO));
...
void FreeGraphicResources()
{
	...
	DeleteObject(g_timoshenkoBitmap);
...
void InitDrawTimer(HWND hWnd)
{
	...
	g_drawTimerId = SetTimer(
			...
			20,				// период (в миллисекундах) между событиями таймера
...
void AlphaBlendPicture(HDC hdc, HBITMAP picture, int x, int y)
{
	BITMAP bmp;
	GetObject(picture, sizeof(bmp), &bmp);

	HDC bitmapDC = CreateCompatibleDC(hdc);
	HGDIOBJ oldBitmap = SelectObject(bitmapDC, picture);
	...
	BOOL blendResult = AlphaBlend(
		hdc,                // дескриптор устройства, на котором мы будем рисовать
		destinationX,		// x-координата верхнего левого угла на области вывода
		destinationY,		// y-координата верхнего левого угла на области вывода
		destinationWidth,	// Ширина прямоугольника на области вывода
		destinationHeight,	// Высота прямоугольника на области вывода
		bitmapDC,			// дескриптор устройства, с которого будет происходить альфа-наложение пикселей
		sourceX,			// x-коодрината на области-источнике пикселей
		sourceY,			// y-коодрината на области-источнике пикселей
		sourceWidth,		// ширина прямоугольника на области-источнике пикселей
		sourceHeight,		// высота прямоугольника на области-источнике пикселей
		blend				// функция альфа-смешивания
	);
...
void DrawTimoshenko(HDC hdc)
{
	int timoshenkoX = 220 + (int)(100 * sin((g_timoshenkoFlyPhaseX % 1000) * 3.1415927f / 500));
	int timoshenkoY = 100 + (int)(100 * sin((g_timoshenkoFlyPhaseY % 1000) * 3.1415927f / 500));
	AlphaBlendPicture(hdc, g_timoshenkoBitmap, timoshenkoX, timoshenkoY);
}
...
void OnTimer(HWND hWnd, int idTimer)
{
	...
	g_timoshenkoFlyPhaseX += 13;
	g_timoshenkoFlyPhaseY += 11;
}
