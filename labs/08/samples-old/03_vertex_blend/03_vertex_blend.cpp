// 03_vertex_blend.cpp : Defines the entry point for the console application.
//
...
void InitShape(float radius, Vertex * vertexArray, unsigned const numberOfVertices)
{
	...
	for (unsigned i = 0; i < numberOfVertices; ++i)
	{
		...
		float angle = 2 * M_PI * i / numberOfVertices;
		v.x0 = float(radius * cos(angle));
		v.y0 = float(radius * sin(angle));
		v.z0 = 0;

		float radius1 = radius * 0.7 + 0.6 * radius * cos(6 * angle);
