...
const TCHAR WINDOW_TITLE[] = "Texture + lighting ";				// The title bar text
...
GLint g_fogMode = GL_LINEAR;
float g_fogDensity = 0.1f;
float g_fogStart = 3.0f;
float g_fogEnd = 15;
Color4d g_fogColor = {1, 1, 1, 1};
...
static const numLights = sizeof(lights) / sizeof(LightDescriptor);
...
static const numMaterials = sizeof(materials) / sizeof(MaterialDecriptor);
...
void OnCommandMessage(WPARAM wParam, LPARAM lParam)
{
	...
	case ID_FOG_NONE:
		g_fogMode = 0;
		break;
	case ID_FOG_LINEARFOG:
		g_fogMode = GL_LINEAR;
		break;
	case ID_FOG_EXPONENTIALFOG:
		g_fogMode = GL_EXP;
		break;
	case ID_FOG_EXP2FOG:
		g_fogMode = GL_EXP2;
		break;
	case ID_FOGCOLOR_WHITE:
		g_fogColor.Set(1, 1, 1, 1);
		break;
	case ID_FOGCOLOR_BLACK:
		g_fogColor.Set(0, 0, 0, 1);
		break;
	case ID_FOGCOLOR_RED:
		g_fogColor.Set(1, 0, 0, 1);
		break;
	case ID_FOGCOLOR_GREEN:
		g_fogColor.Set(0, 1, 0, 1);
		break;
	case ID_FOGCOLOR_BLUE:
		g_fogColor.Set(0, 0, 1, 1);
		break;
	default:
		break;
	}
