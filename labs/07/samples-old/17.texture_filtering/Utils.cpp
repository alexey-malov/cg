...
GLuint LoadTextureMipLevelFromRgbBitmapFile(const char fileName[], int mipLevel)
{
	FILE *pFile = fopen(fileName, "rb");
	if (!pFile)
	{
	...
	*/
	...
	glTexImage2D(GL_TEXTURE_2D, mipLevel, numberOfComponents == 3 ? GL_RGB : GL_RGBA, width, height, 0, numberOfComponents == 3 ? GL_BGR_EXT : GL_BGRA_EXT, GL_UNSIGNED_BYTE, pBuffer);
	delete [] pBuffer;
	return true;
}
