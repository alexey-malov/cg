...
{
	return (degrees + seconds / 60) * M_PI / 180;
}
...
{
	DWORD timeDelta = m_animationController.GetTimeDelta();
	m_animationController.Tick();

	m_time += timeDelta * 0.001 * m_timeSpeed;

	int year = floor(m_time);
...
void CMainApp::DrawPlanets()
{
	for (int i = 0; i < PLANET_COUNT; ++i)
	{
		...
		glEnable(GL_POINT_SMOOTH);
		glPointSize(planet.size * sqrt(m_scale) / 4);
		glBegin(GL_POINTS);
		{
