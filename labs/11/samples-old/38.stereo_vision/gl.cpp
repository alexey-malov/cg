...
const TCHAR WINDOW_TITLE[] = "Terrain - part1";				// The title bar text
...
{
	// устанавливаем матрицу проецировани€ (aspect - отношение ширины экрана к его высоте)
...
void ShiftTime()
{
	// подсчитываем врем€, прошедшее с момента последнего нарисованного кадра
...
bool InitMapTile()
{
	...
	GLuint texDetail = LoadTextureFromRgbBitmapFile("detail.bmp");
	if (!texDetail)
	{
		return false;
	}
...
bool InitSkySphere()
{
	g_skyTexture = LoadTextureFromRgbBitmapFile("sky.bmp");
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
